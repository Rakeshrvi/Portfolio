    ...
    return (
        <div ref={ref} className={`mb-8 flex justify-between items-center w-full ${index % 2 === 0 ? 'flex-row-reverse md:flex-row' : 'flex-row-reverse'} transition-all duration-700 ${animationClasses} ${delayClass}`}>
            ...
            <div className="z-20 flex items-center order-1 bg-blue-500 shadow-xl w-8 h-8 rounded-full">
                <div className="mx-auto text-white font-semibold text-lg">{/* Icon can go here */}</div>
            </div>
            <div className={`order-1 bg-white border border-blue-200 rounded-lg shadow-xl w-5/12 px-6 py-4 ${contentSide}`}>
                <p className="text-sm font-semibold text-blue-600 mb-1">{item.date}</p>
                <h3 className="font-bold text-slate-900 text-lg mb-1">{item.title}</h3>
                <p className="text-sm font-medium text-slate-700 mb-2">{item.organization}</p>
                <p className="text-sm leading-snug tracking-wide text-slate-600 text-opacity-100 text-justify">{item.description}</p>
            </div>
        </div>
    );
};
...
const TimelineItemMobile: React.FC<{ item: VolunteeringItem; index: number }> = ({ item, index }) => {
    ...
    return (
        <div ref={ref} className={`relative pl-8 transition-all duration-700 ${animationClasses} delay-${index * 150}`}>
            <div className="absolute left-0 top-1 h-full border-l-2 border-blue-200"></div>
            <div className="absolute left-[-9px] top-1 w-4 h-4 bg-white border-2 border-blue-500 rounded-full"></div>
            <div className="bg-white border border-blue-200 rounded-lg p-4 mb-6 shadow-lg">
                <p className="text-sm font-semibold text-blue-600 mb-1">{item.date}</p>
                <h3 className="font-bold text-slate-900 text-lg mb-1">{item.title}</h3>
                <p className="text-sm font-medium text-slate-700 mb-2">{item.organization}</p>
                <p className="text-sm leading-snug tracking-wide text-slate-600 text-opacity-100 text-justify">{item.description}</p>
...
const ResponsiveTimeline: React.FC = () => {
    ...
    return (
        <div className="relative wrap overflow-hidden p-2 md:p-10 h-full">
            <div className="absolute border-opacity-40 border-blue-300 h-full border" style={{ left: '50%' }}></div>
...
const VolunteeringFC: React.FC = () => {
    const { ref, animationClasses } = useScrollAnimation<HTMLDivElement>();
    return (
        <section id="volunteering" className="py-20 md:py-32 bg-blue-50">
